...
if [ -e $JOB.pnc ] ; then mv -f $JOB.pnc $JOB.pnc.bak ; fi
if [ -e $JOB.out ] ; then mv -f $JOB.out $JOB.out.bak ; fi
if [ -e $HOMEDIR/$JOB.xyz ]  ; then rm -f $SCR/$JOB.xyz  ; cp $HOMEDIR/$JOB.xyz  $SCR/ ; fi
if [ -e $HOMEDIR/frag.key ]  ; then rm -f $SCR/frag.key  ; cp $HOMEDIR/frag.key  $SCR/ ; fi
if [ -e $HOMEDIR/amber.prm ] ; then rm -f $SCR/amber.prm ; cp $HOMEDIR/amber.prm $SCR/ ; fi

[ -z "${NP:-}" ] && NP=1

if (( `grep -i NELONG= $JOB.inp | wc -l` > 0 ))
then
	ELGNAME=$4
	[ -z "${4:-}" ] && ELGNAME=ELGFILE
	export AOINTS=$SCR/$ELGNAME.F08
	export AOINTS=$SCR/$ELGNAME.FELGDOS=~$USER/scr/$JOB.ldos
	export AOINTS=$SCR/$ELGNAME.FELGDAT=$SCR/$ELGNAME.F71
	export AOINTS=$SCR/$ELGNAME.FELGPAR=$SCR/$ELGNAME.F72
	export AOINTS=$SCR/$ELGNAME.FELGCUT=$SCR/$ELGNAME.F74
	export AOINTS=$SCR/$ELGNAME.FELGVEC=$SCR/$ELGNAME.F75
	export AOINTS=$SCR/$ELGNAME.FEGINTA=$SCR/$ELGNAME.F77
	export AOINTS=$SCR/$ELGNAME.FEGINTB=$SCR/$ELGNAME.F78
	export AOINTS=$SCR/$ELGNAME.FEGTDHF=$SCR/$ELGNAME.F79
	export AOINTS=$SCR/$ELGNAME.FEGTEST=$SCR/$ELGNAME.F80
else
	export AOINTS=$SCR/$JOB.F08
fi
export ERICFMT=$GAMESS_HOME/auxdata/ericfmt.dat
export MCPPATH=$GAMESS_HOME/auxdata/MCP
export EXTBAS=$GAMESS_HOME/BASIS.LIB
export NUCBAS=/dev/null
export FRAGFILE=$SCR/$JOB.xyz
export FRAGOUT=$SCR/$JOB.fgout
export FORCEFILE=$SCR/$JOB.dat
export MAKEFP=$SCR/$JOB.efp
export GAMMA=$SCR/$JOB.gamma
export TRAJECT=$SCR/$JOB.trj
export RESTART=$SCR/$JOB.rst
export INPUT=$HOMEDIR/$JOB.inp
export PUNCH=$HOMEDIR/$JOB.pnc
export DMOUT=$HOMEDIR/$JOB.dmout
...
mpirun -n $NP $GAMESS_HOME/gamess.00.x >& $HOMEDIR/$JOB.out.${NP}.${NTHR}
